...
Enhanced with Bug Bounty & Penetration Testing Features
Multi-File Project Management, AI Code Review, Debugging & More
"""
            ...
            try:
                ...
                for item in items:
                    ...
                    if item.is_dir():
                        ...
                        if len(list(item.iterdir())) < 50:  # Prevent too deep trees
...
TASK = user wants you to DO something (run commands, create files, analyze data, edit files, security testing, etc.)

Examples:
"hi" -> CHAT
"how are you?" -> CHAT
"thanks" -> CHAT
"scan for vulnerabilities" -> TASK
...
Execute the task now. Use appropriate <action> tags."""
        ...
        system_prompt = f"""You are an advanced autonomous AI assistant with comprehensive capabilities.
...
<action>{{"type": "edit_file", "path": "file.py", "instructions": "add login"}}</action>
<action>{{"type": "create_file", "path": "file.py", "description": "create login page"}}</action>

🔒 SECURITY TESTING (Bug Bounty & Pentesting):
...
❓ ASK USER:
<question>What information do you need?</question>

USER REQUEST: {user_input}

ANALYZE AND EXECUTE:
1. Understand the request
2. Determine if you have all needed information
...
╔════════════════════════════════════════════════════════════════════════╗
║                                                                        ║
║       🚀 ADVANCED AUTONOMOUS AI ASSISTANT - Bug Bounty Edition         ║
║                                                                        ║
...
║  • "analyze app.py for security vulnerabilities"                       ║
║  • "create a Flask REST API project"                                   ║
