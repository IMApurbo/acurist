...
def ask_ai(bot, history, user_msg, sys_prompt):
    ...
    # Build conversation block (last MAX_HISTORY messages only)
    conv_lines = []
    ...
    full_prompt = (
        ...
        f"{conv_block}\n"
        f"{'═'*60}\n"
        f"ASSISTANT (respond with ONLY a JSON object, nothing else):"
    )
    ...
    # Store only the raw AI reply in history (not the full prompt)
