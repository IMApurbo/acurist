...
VERSION       = "4.0.0"   # bump this to bust __pycache__
...
# Full automated recon: subdomains → ports → web tech → vulns
# ══════════════════════════════════════════════════════════════

class ReconPipeline:
    """
    /recon <domain> — runs a staged automated recon pipeline.
    Each stage feeds results into the next.
    Stages: subdomains → live check → port scan → web tech → nuclei
    """
    TOOLS_APT = {
        "subfinder": None,  # go tool
        "httpx":     None,
        "nmap":      "nmap",
        "whatweb":   "whatweb",
        "nuclei":    None,
    ...
    def _run(self, cmd: str, desc: str) -> str:
        """Run a shell command, stream output, return stdout."""
        print(c("cyan", f"\n  ┌─ [{desc}]"))
        print(c("dim",  f"  │  $ {cmd}"))
        lines = []
        try:
    ...
    def _ensure(self, tool: str):
        if not self._has(tool):
            print(c("yellow", f"  [recon] installing {tool}..."))
            installer = ToolInstaller(CommandExecutor())
    ...
    def run(self, domain: str) -> str:
        ...
        report_parts = [f"# Recon Report: {domain}", f"*{datetime.now().strftime('%Y-%m-%d %H:%M')}*", ""]
        ...
        if self._has("subfinder"):
            subs_raw += self._run(f"subfinder -d {domain} -silent -o {outdir}/subfinder.txt 2>/dev/null", "subfinder")
        if self._has("sublist3r"):
            subs_raw += self._run(f"sublist3r -d {domain} -o {outdir}/sublist3r.txt 2>/dev/null", "sublist3r")
        # merge + deduplicate
        self._run(
            f"cat {outdir}/subfinder.txt {outdir}/sublist3r.txt 2>/dev/null | sort -u > {subs_file}",
            "merge subdomains"
        )
        subs_count = len([l for l in subs_raw.splitlines() if l.strip()])
        report_parts += [f"## Subdomains Found: {subs_count}", f"```", subs_raw[:2000] or "none", "```", ""]
        self.results["subdomains"] = subs_raw

        # ── Stage 2: Live host check ──────────────────────────
        print(c("magenta", "\n━━ Stage 2: Live Host Check ━━"))
        live_file = f"{outdir}/live.txt"
        live_raw  = ""
        if self._has("httpx") and os.path.exists(subs_file):
            live_raw = self._run(
                f"httpx -l {subs_file} -silent -o {live_file} 2>/dev/null",
                "httpx live check"
            )
        elif os.path.exists(subs_file):
            self._run(f"cat {subs_file} | head -20", "preview subdomains")
        report_parts += [f"## Live Hosts", f"```", live_raw[:1000] or "skipped (httpx not found)", "```", ""]

        # ── Stage 3: Port scan on main domain ─────────────────
        print(c("magenta", "\n━━ Stage 3: Port Scan ━━"))
        port_raw = ""
        if self._has("nmap"):
            port_raw = self._run(
                f"nmap -T4 -F --open {domain} 2>/dev/null",
                "nmap fast scan"
            )
        else:
            port_raw = self._run(f"nc -zv {domain} 80 443 8080 8443 22 21 2>&1 | grep -i open", "nc port check")
        report_parts += ["## Port Scan", "```", port_raw[:1000] or "no open ports", "```", ""]
        self.results["ports"] = port_raw

        # ── Stage 4: Web tech fingerprint ─────────────────────
        print(c("magenta", "\n━━ Stage 4: Web Tech Fingerprint ━━"))
        tech_raw = ""
        if self._has("whatweb"):
            tech_raw = self._run(f"whatweb -a 3 {domain} 2>/dev/null", "whatweb")
        elif self._has("curl"):
            tech_raw = self._run(f"curl -sI https://{domain} 2>/dev/null | head -20", "curl headers")
        report_parts += ["## Web Technologies", "```", tech_raw[:500] or "none", "```", ""]

        # ── Stage 5: Nuclei vulnerability scan ────────────────
        print(c("magenta", "\n━━ Stage 5: Vulnerability Scan ━━"))
        vuln_raw = ""
        if self._has("nuclei"):
            vuln_raw = self._run(
                f"nuclei -u https://{domain} -severity medium,high,critical -silent 2>/dev/null",
                "nuclei"
            )
        else:
            print(c("dim", "  [recon] nuclei not found — skipping vuln scan"))
        report_parts += ["## Vulnerabilities", "```", vuln_raw[:2000] or "none / nuclei not installed", "```", ""]
        ...
        print(c("green", "\n  ╔══ RECON COMPLETE ═══════════════════════════════════════"))
        print(c("green", f"  ║  Report saved: {report_path}"))
        print(c("green", f"  ╚{'═'*60}"))
        return report_md
