...
class CommandExecutor:
    def run(self, command: str, timeout: int = 180) -> dict:
        ...
        try:
            ...
            print(c("dim", f"  └─ {icon} exit:{process.returncode} ({elapsed}s)"))
            return {
                "command":    command,
                "stdout":     "\n".join(stdout_lines),
                "stderr":     "\n".join(stderr_lines),
            ...
            return {"command": command, "stdout": "", "stderr": "Timeout",
                    "returncode": -1, "success": False, "elapsed": timeout}
        except Exception as e:
            return {"command": command, "stdout": "", "stderr": str(e),
                    "returncode": -1, "success": False, "elapsed": 0}
...
class ExecutionEngine:
    ...
    def execute_plan(self, plan: dict, task_id: str) -> str:
        ...
        for step in steps:
            ...
            if stype == "info":
                results.append(f"[Step {sid}] {desc}")
                self.memory.log_task_step(task_id, sid, "", "", desc, "info")
                continue

            if not cmd:
                print(c("yellow", "  ⚠ No command provided for this step."))
                continue

            result = self._run_with_healing(
                cmd, stype, task_id, sid, step.get("tool", "")
            )
    ...
    def _run_with_healing(self, command: str, stype: str,
        ...
        for attempt in range(1, MAX_RETRIES + 1):
            if stype == "python":
                ...
                result = self.cmd_exec.run(command)
            ...
            if attempt < MAX_RETRIES:
                print(c("yellow",
                        f"  ⚠ Attempt {attempt}/{MAX_RETRIES} failed — self-healing..."))
                error_text = result["stderr"] or result["stdout"]
                fixed = self.analyzer.analyze_and_fix(command, error_text, history)
                if fixed and fixed.strip() != command.strip():
                    print(c("magenta", f"  ↺ Rebuilt: {fixed}"))
                    command = fixed
                else:
                    print(c("red", "  ✗ No fix found, moving on."))
                    break
            else:
