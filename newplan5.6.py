...
VERSION       = "4.2.0"   # bump this to bust __pycache__
...
class ToolInstaller:
    ...
    def install(self, tool: str) -> tuple:
        ...
        ok = self._shell(f"pip install {pip_pkg} --break-system-packages -q")
        if ok or shutil.which(tool):
...
class ExecutionEngine:
    ...
    def _run_with_healing(self, command: str, stype: str,
        ...
        if stype != "python" and self._is_install_cmd(command):
            ...
            ret = os.system(f"pip install {pip_pkg} --break-system-packages")
...
class ReconPipeline:
    ...
    REQUIRED_TOOLS = [
        ...
        ("nmap",            "port scanner"),
        ("gobuster",        "directory/vhost brute-force"),
    ...
    # apt package name if different from binary name
    APT_NAMES = {
        "subfinder":     "subfinder",
        "sublist3r":     "sublist3r",
        "amass":         "amass",
    ...
    # ── Stage 0: Install all required tools via apt ───────────
    def _install_all_tools(self):
        ...
        print(c("yellow", f"\n  ╭─ Installing {len(missing)} missing tools via apt"))
        for tool in missing:
            apt_pkg = self.APT_NAMES.get(tool, tool)
            print(c("dim", f"  │  apt-get install -y {apt_pkg}"))

        # Update once, then install all in one apt call for speed
        ...
        for tool in missing:
            apt_pkg = self.APT_NAMES.get(tool, tool)
            print(c("cyan", f"\n  │  Installing {apt_pkg}..."))
            ret = os.system(f"apt-get install -y {apt_pkg} 2>&1")
            if ret == 0 or self._has(tool):
                print(c("green", f"  │  ✓ {tool} installed"))
            else:
                print(c("red", f"  │  ✗ {tool} failed — will skip in pipeline"))

        print(c("green", "  ╰─ Tool installation complete\n"))
    ...
    def run(self, domain: str) -> str:
        ...
        try:
            ...
            if self._has("sublist3r"):
                self._run(
                    f"sublist3r -d {domain} -o {outdir}/sublist3r.txt -v 2>/dev/null",
                    "sublist3r", timeout=180
                )
            if self._has("amass"):
                self._run(
                    f"amass enum -passive -d {domain} -o {outdir}/amass.txt 2>/dev/null",
                    "amass passive", timeout=180
                )
            # Merge all subdomain files, deduplicate
            os.system(
                f"cat {outdir}/subfinder.txt {outdir}/sublist3r.txt {outdir}/amass.txt "
        ...
        print(c("yellow", "\n  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"))
        print(c("yellow",  "  ▶ Stage 3 — WAF Detection (wafw00f)"))
        print(c("yellow",  "  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"))
        waf_raw = ""
        try:
            if self._has("wafw00f"):
                waf_raw = self._run(
                    f"wafw00f https://{domain} 2>/dev/null",
                    "wafw00f WAF detection", timeout=60
                )
            report_parts += ["## WAF Detection", "```", waf_raw[:500] or "none", "```", ""]
        except KeyboardInterrupt:
            print(c("yellow", "  ⚡ Stage 3 skipped"))
            report_parts += ["## WAF Detection", "```", "skipped", "```", ""]

        # ══ Stage 4: Port Scan ════════════════════════════════
        print(c("yellow", "\n  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"))
        print(c("yellow",  "  ▶ Stage 4 — Port Scan (nmap)"))
        print(c("yellow",  "  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"))
        port_raw = ""
        try:
            if self._has("nmap"):
                port_raw = self._run(
                    f"nmap -T4 -sV --open -p 21,22,23,25,53,80,110,143,443,445,"
                    f"3306,3389,5432,6379,8080,8443,8888,9200,27017 {domain} 2>/dev/null",
                    "nmap service scan", timeout=240
                )
            report_parts += ["## Port Scan", "```", port_raw[:2000] or "no open ports", "```", ""]
            self.results["ports"] = port_raw
        except KeyboardInterrupt:
            print(c("yellow", "  ⚡ Stage 4 skipped"))
            report_parts += ["## Port Scan", "```", "skipped", "```", ""]

        # ══ Stage 5: Directory Brute-Force ════════════════════
        print(c("yellow", "\n  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"))
        print(c("yellow",  "  ▶ Stage 5 — Directory Brute-Force (gobuster)"))
        print(c("yellow",  "  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"))
        dir_raw = ""
        try:
            ...
            if self._has("gobuster") and os.path.exists(wordlist):
                dir_raw = self._run(
                    f"gobuster dir -u https://{domain} -w {wordlist} "
                    f"-t 50 -q --no-error 2>/dev/null",
                    "gobuster dir brute-force", timeout=300
                )
            elif self._has("gobuster"):
                print(c("dim", "  [!] No wordlist found — install seclists or dirb"))
            report_parts += ["## Directories Found", "```", dir_raw[:2000] or "none", "```", ""]
        except KeyboardInterrupt:
            print(c("yellow", "  ⚡ Stage 5 skipped"))
            report_parts += ["## Directories Found", "```", "skipped", "```", ""]

        # ══ Stage 6: Web Tech Fingerprint ════════════════════
        print(c("yellow", "\n  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"))
        print(c("yellow",  "  ▶ Stage 6 — Web Tech Fingerprint (whatweb)"))
        print(c("yellow",  "  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"))
        tech_raw = ""
        try:
            if self._has("whatweb"):
                tech_raw = self._run(
                    f"whatweb -a 3 --no-errors https://{domain} 2>/dev/null",
                    "whatweb tech fingerprint", timeout=60
                )
            report_parts += ["## Web Technologies", "```", tech_raw[:1000] or "none", "```", ""]
        except KeyboardInterrupt:
            print(c("yellow", "  ⚡ Stage 6 skipped"))
            report_parts += ["## Web Technologies", "```", "skipped", "```", ""]

        # ══ Stage 7: Nikto Web Vulnerability Scan ════════════
        print(c("yellow", "\n  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"))
        print(c("yellow",  "  ▶ Stage 7 — Web Vulnerability Scan (nikto)"))
        print(c("yellow",  "  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"))
        nikto_raw = ""
        try:
            if self._has("nikto"):
                nikto_raw = self._run(
                    f"nikto -h https://{domain} -nointeractive 2>/dev/null",
                    "nikto web scan", timeout=300
                )
            report_parts += ["## Nikto Findings", "```", nikto_raw[:3000] or "none", "```", ""]
        except KeyboardInterrupt:
            print(c("yellow", "  ⚡ Stage 7 skipped"))
            report_parts += ["## Nikto Findings", "```", "skipped", "```", ""]

        # ══ Stage 8: Nuclei Vulnerability Scan ═══════════════
        print(c("yellow", "\n  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"))
        print(c("yellow",  "  ▶ Stage 8 — Template Vuln Scan (nuclei)"))
        print(c("yellow",  "  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"))
        nuclei_raw = ""
        try:
            if self._has("nuclei"):
                ...
                nuclei_raw = self._run(
                    f"nuclei -u https://{domain} -severity low,medium,high,critical "
                    f"-silent -o {outdir}/nuclei.txt 2>/dev/null",
                    "nuclei vuln scan", timeout=300
                )
            report_parts += ["## Nuclei Vulnerabilities", "```",
                             nuclei_raw[:3000] or "none found", "```", ""]
        except KeyboardInterrupt:
            print(c("yellow", "  ⚡ Stage 8 skipped"))
            report_parts += ["## Nuclei Vulnerabilities", "```", "skipped", "```", ""]
        ...
        print(c("green", "\n  ╔══ RECON COMPLETE ══════════════════════════════════════════"))
        print(c("green", f"  ║  Target  : {domain}"))
        print(c("green", f"  ║  Report  : {report_path}"))
        print(c("green", f"  ║  Files   : {outdir}/"))
        print(c("green", f"  ╚{'═'*62}"))
        print(c("dim",   f"  Tip: view full report with:  cat {report_path}"))
        print()
        return report_md
