...
VERSION       = "5.8.1"   # bump this to bust __pycache__
...
class CommandExecutor:
    ...
    def as_user(command: str) -> str:
        """
        Wrap command to run as the real (non-root) user via su.
        Used for GUI apps, browsers, desktop tools that refuse root.
        """
        sudo_user = os.environ.get("SUDO_USER")
        if not sudo_user or os.geteuid() != 0:
            return command
        # Preserve DISPLAY and XAUTHORITY for GUI apps
        env_prefix = (
...
class PythonExecutor:
    """
    Code generator + runner for tasks that need Python.

    Flow:
      1. generate_and_run() asks the LLM for ONLY Python3 code
         (strict system-prompt, no JSON, no fences, no prose)
      2. Code is written verbatim to a named temp file
      3. Imports are scanned — any missing pip packages auto-installed
      4. Script is executed, output streamed live
      5. Temp file deleted whether it succeeds or fails

    Retry: on failure the error is fed back and a fresh script is generated.
    """
    ...
    def __init__(self):
        ...
        self._installed: set = set()   # packages installed this session

    # ─────────────────────────────────────────────────────────
    # PRIMARY entry point — generate code then run it
    # ─────────────────────────────────────────────────────────
    def generate_and_run(self, task: str, user_input: str,
                         cwd: str = "/root",
                         model: str = DEFAULT_MODEL) -> dict:
        """
        Ask the LLM for Python3 code only, install deps, execute, clean up.
        """
        print(c("cyan", f"\n  [CodeGen] ✎ Generating script: {task[:70]}"))
        agent = FreeLLM(model=model)

        prompt = (
            "### SYSTEM: Python3 code generator — strict output rules ###\n"
            "Output ONLY valid Python3 source code.\n"
            "FORBIDDEN: markdown fences (```), prose, explanations, blank intro lines.\n"
            "REQUIRED: first line = a Python statement (import/def/class/assignment).\n"
            "\n"
            f"### TASK ###\n{task}\n"
            f"\n### USER REQUEST ###\n{user_input}\n"
            f"\n### WORKING DIRECTORY ###\n{cwd}\n"
            "\n### CODE RULES ###\n"
            "- Standalone script: python3 script.py must work with no arguments\n"
            "- All imports at the top\n"
            f"- Files mentioned in the task/request: use their EXACT paths as given\n"
            f"- If no path given for OUTPUT files: save to {cwd}/\n"
            "- NEVER assume /tmp for file paths unless explicitly told\n"
            "- os.makedirs(os.path.dirname(path), exist_ok=True) before any file write\n"
            "- Last line: print a clear success message with the output file path\n"
        )
        ...
        if not code:
            return {"command": "codegen", "stdout": "", "elapsed": 0,
                    "stderr": "LLM returned no code", "returncode": -1,
                    "success": False, "cancelled": False}

        return self._install_and_run(code, cwd=cwd)

    # ─────────────────────────────────────────────────────────
    # Install missing deps detected from imports, then run
    # ─────────────────────────────────────────────────────────
    def _install_and_run(self, code: str, cwd: str = None) -> dict:
        # 1. Scan imports and install anything missing
        missing = self._find_missing_imports(code)
        for pkg_import, pip_pkg in missing:
            if pip_pkg in self._installed:
                continue
            print(c("yellow", f"  [CodeGen] Installing: {pip_pkg}"))
            os.system(
                f"pip install {pip_pkg} --break-system-packages -q "
                f"--root-user-action=ignore 2>/dev/null"
            )
            self._installed.add(pip_pkg)

        # 2. Write to named temp file
        tmp_path = os.path.join(tempfile.gettempdir(),
                                f"hackers_ai_{os.getpid()}_{int(time.time())}.py")
        with open(tmp_path, 'w', encoding='utf-8') as f:
            f.write(code)

        print(c("dim", f"  [CodeGen] Running: {tmp_path}"))

        # 3. Execute
        result = self.executor.run(f"python3 {tmp_path}", timeout=120, cwd=cwd)
    ...
    def _extract_code(raw: str) -> str:
        """Strip markdown fences and any leading prose lines."""
        # Remove ```python ... ``` or ``` ... ```
        raw = re.sub(r"^```[a-zA-Z0-9]*\s*\n?", "", raw.strip(), flags=re.IGNORECASE)
        raw = re.sub(r"\n?```\s*$", "", raw).strip()

        # Drop any leading lines that don't look like Python
        lines = raw.splitlines()
        for i, line in enumerate(lines):
            s = line.strip()
            if s and (
                s.startswith(("import ", "from ", "def ", "class ", "#!", "#",
                               "if ", "for ", "with ", "try:", "os.", "print(")) or
                re.match(r'^[a-zA-Z_]\w*\s*(=|\()', s)
            ):
                return "\n".join(lines[i:]).strip()
        return raw.strip()

    def _find_missing_imports(self, code: str):
        """Return list of (import_name, pip_package) for uninstallable imports."""
...
def extract_json(text: str) -> Optional[dict]:
    ...
    # ── Truncation recovery: JSON was cut off before closing ────
    # Find the opening { and try to auto-close the structure
    ...
    if first_brace != -1:
        ...
        # Fix trailing commas before auto-closing
...
LIVE SYSTEM DATA:
  ...
  HOME     : {profile.get('real_home', '/root')}  (use this, NOT ~ or /root, in commands)
  Tools    : {tools_str}

DIRECTORY RULE: When the user says "here", "this folder", "current directory", or gives a
relative path, resolve it relative to CWD shown above. When no path is given, use CWD.
...
13. CRITICAL — Tasks that need Python (file creation, PDFs, code generation, data processing):
    - Use type="python" with command="brief description of what to do"
    - The Python script will be AI-generated at execution time — do NOT write code in JSON
    - The Python script handles EVERYTHING end-to-end: install libs, create the file, confirm done
    - NEVER add a separate compile/pdflatex/pandoc step after a python step — python does it all
    - Pattern: step1=mkdir if needed (type=command), step2=do the whole task (type=python)
    - ONE python step per logical task — do not split into generate+compile
14. Keep your JSON compact — no extra blank lines or indentation inside the JSON.
...
depends_on: list of step IDs this step must wait for. Empty [] means it can run in parallel.
    ...
    # ── Detect if a step needs file content generated separately ─
    _FILE_WRITE_TRIGGERS = (
        "html", "css", "javascript", "js", "python script", "bash script",
        "config", "write a file", "create a file", "index.html", "page",
        "webpage", "website", "script", "template",
    )

    def _needs_content_expansion(self, user_input: str) -> bool:
        ui = user_input.lower()
        return any(t in ui for t in self._FILE_WRITE_TRIGGERS)

    def _expand_file_step(self, step: dict, user_input: str,
                          model: str = DEFAULT_MODEL,
                          cwd: str = "/root") -> dict:
        """
        Generate a complete Python3 script for any PLACEHOLDER step.
        Works for ALL tasks: writing files, C code, scripts, algorithms, etc.
        Separate LLM call — no JSON overhead, no size limit.
        """
        desc   = step.get("description", "")
        agent  = FreeLLM(model=model)
        script = agent.ask(
            f"Write a complete, standalone Python3 script for this task:\n"
            f"Task: {desc}\n"
            f"User request: {user_input}\n"
            f"Current working directory: {cwd}\n\n"
            f"Rules:\n"
            f"- Output ONLY raw Python3 code — no markdown fences, no explanation\n"
            f"- Script must be fully self-contained and runnable with: python3 script.py\n"
            f"- For writing files: save to {cwd}/<filename> using open().write()\n"
            f"  Use os.makedirs(os.path.dirname(path), exist_ok=True) before writing\n"
            f"- For code generation (C, bash, etc): write the code to a file in {cwd}\n"
            f"- Print a clear confirmation at the end, e.g.: print('[OK] Created: /path/to/file')\n"
        ).strip()
    ...
    def plan(self, user_input: str, history: list, profile: dict,
        ...
        base_parts.append(
            f"[NEW TASK — plan ONLY for this, ignore all previous tasks]: {user_input}\n"
            f"IMPORTANT: For steps that need Python (file creation, code generation, algorithms), "
            f"use type=python and set command to a SHORT description of what the script should do. "
            f"Do NOT write actual Python code in the JSON — it will be generated separately."
        )
        prompt = "\n".join(base_parts)
        ...
        for attempt in range(1, 4):
            try:
                ...
                if attempt > 1:
                    ...
                    print(c("dim", f"  [Planner raw attempt {attempt}]: {preview}"))

                plan = extract_json(raw)
                if plan and isinstance(plan.get("steps"), list) and plan["steps"]:
                    # Python steps: command field is just a description hint now.
                    # Actual code is generated fresh at execution time by CodeGen.
                    # Just ensure every python step has a description for CodeGen to use.
                    for step in plan["steps"]:
                        if step.get("type") == "python":
                            if not step.get("description"):
                                step["description"] = step.get("command", user_input)
                            # Clear command — CodeGen will generate real code
                            step["command"] = ""
                    return plan
                ...
                prompt += (
                    ...
                    "For python steps set command to a brief description, not actual code. "
...
You are Hackers AI — a powerful Linux agent with FULL root access to a real Kali machine.
You can read files, run commands, and take action on ANYTHING on this system.

LIVE SYSTEM:
  OS       : {profile.get('distro', 'Linux')}
  Kernel   : {profile.get('kernel', '')}
  Hostname : {profile.get('hostname', '')} | User: {profile.get('whoami', '')} | Root: True
  CPU      : {profile.get('cpu', '')}
  RAM      : {profile.get('ram', '')}
  Disk     : {profile.get('disk', '')}
  IP       : {profile.get('ip', '')}
  CWD      : {profile.get('cwd', '/root')}
  Home     : {profile.get('real_home', '/home/kali')}
  Tools    : {tools_str}

STRICT RULES:
- NEVER say "I can't", "I don't have access", "I'm unable to", or apologize for limitations
- NEVER suggest the user open a GUI app — you have shell and Python, use them
- If asked about a file: tell the user the exact command to read it OR just describe what to do
- If a task needs execution: say "run: <command>" — be concrete and actionable
- Keep answers SHORT and direct. No unnecessary prose.
- Use the conversation history to understand what file/target the user is referring to
        """).strip()

        # Only last 4 history turns, truncated, to give context without bloat
        parts = [system_ctx, "\n--- RECENT CONVERSATION ---"]
        for h in history[-4:]:
            prefix  = "USER" if h["role"] == "user" else "AI"
        ...
        agent    = FreeLLM(model=self.model)
        response = agent.ask("\n".join(parts))
        # Strip any accidental repetition of the USER line if LLM echoes it
...
class ExecutionEngine:
    ...
    def __init__(self, memory: MemoryDB, model: str = DEFAULT_MODEL):
        ...
        self._cwd        = None           # set by CLI before execute_plan()
        self._user_input = ""             # set by CLI — passed to CodeGen
    ...
    @staticmethod
    def _expand_tilde(cmd: str) -> str:
        """
        Expand ~ to the REAL user home (SUDO_USER), not /root.
        Called on every command before execution.
        """
        if '~' not in cmd:
            return cmd
        sudo_user = os.environ.get("SUDO_USER")
        real_home = os.path.expanduser(f"~{sudo_user}") if sudo_user \
                    else os.path.expanduser("~")
        # Replace ~/ and standalone ~
        cmd = re.sub(r'(?<![\w])~(?=/|\s|$)', real_home, cmd)
        return cmd
    ...
    def _run_with_healing(self, command: str, stype: str,
        ...
        for attempt in range(1, MAX_RETRIES + 1):
            if stype == "python":
                # Use generate_and_run on first attempt — cleaner than pre-expanded code
                if attempt == 1:
                    result = self.py_exec.generate_and_run(
                        task       = desc or command[:120],
                        user_input = getattr(self, "_user_input", command),
                        cwd        = getattr(self, "_cwd", None) or "/root",
                        model      = self.model,
                    )
                else:
                    # On retry, re-generate with error context appended
                    retry_task = (f"{desc or command[:120]}\n"
                                  f"Previous attempt failed with: {last_err[:200]}")
                    result = self.py_exec.generate_and_run(
                        task       = retry_task,
                        user_input = getattr(self, "_user_input", command),
                        cwd        = getattr(self, "_cwd", None) or "/root",
                ...
                _cmd = self._expand_tilde(command)
                # GUI apps must run as real user, not root
                _GUI = {
                    ...
                    "okular","zathura","code","wireshark","burpsuite","zaproxy",
                ...
                result = self.cmd_exec.run(_cmd, label=label, lock=lock,
            ...
            # Detect "running as root not supported" for GUI apps
            root_gui_err = ("running" in last_err.lower() and "root" in last_err.lower()
                            and ("not supported" in last_err.lower()
                                 or "xauthority" in last_err.lower()))
            if root_gui_err:
                sudo_user = os.environ.get("SUDO_USER")
                if sudo_user:
                    _lp(c("yellow", f"  ⚠ GUI app needs real user — retrying as {sudo_user}..."))
                    _cmd_as_user = CommandExecutor.as_user(command)
                    result = self.cmd_exec.run(_cmd_as_user, label=label, lock=lock)
                    if result["success"]:
                        return result
                    _lp(c("red", f"  ✗ Still failed as {sudo_user}"))
                break
        ...
        # ── Classify failure — soft errors don't deserve escalation ──────────
        _stdout_lower = result.get("stdout", "").lower()
        _stderr_lower = last_err.lower()
        _soft_patterns = [
            "no such file or directory", "cannot access", "not found",
            "no matches found", "0 directories, 0 files", "total 0",
            "ls: cannot", "error opening dir", "is empty",
        ]
        _is_soft = any(p in _stderr_lower or p in _stdout_lower for p in _soft_patterns)
        # ls/find exit:2 with no path means "path doesn't exist" — soft
        _is_soft = _is_soft or (
            result.get("returncode") == 2 and
            any(t in command.split()[0] for t in ["ls", "find"])
        )
        if _is_soft:
            _lp(c("yellow", "  ⚠ Path not found or empty — reporting as-is."))
            result["success"]    = True
            result["stdout"]     = (last_err.strip() or
                                    result.get("stdout","").strip() or
            ...
            if bin_name and not shutil.which(bin_name):
                ...
                if ok:
                    ...
                    result = self.cmd_exec.run(
                        self._expand_tilde(clean), label=label, lock=lock)
        ...
        if stype != "python":
            ...
            if alt_cmd:
                ...
                alt_cmd = self._expand_tilde(alt_cmd)
...
class IntentClassifier:
    ...
    # References to recent context that imply acting on a file/result
    CONTEXT_REF_PATTERNS = [
        r"\b(this|that|the|its|it)\b.{0,30}\b(file|pdf|doc|txt|log|script|code|output|result|dir|folder|image|report)\b",
        r"\b(what|whats|what\'s)\b.{0,20}\b(inside|in|content|contain|say|says|show|include|written)\b",
        r"\b(read|open|view|display|print|show|cat|parse|extract|summarize|analyze)\b.{0,30}\b(it|this|that|the)\b",
        r"\b(inside|content|contain|what.{0,5}in)\b",
    ]
    ...
    def classify(cls, text: str) -> str:
        ...
        # Pure greetings — only these exact words
        ...
        # References to a file/result in context → needs action
        for pat in cls.CONTEXT_REF_PATTERNS:
            if re.search(pat, lower):
                return "task"

        # Question about system → task
        if re.search(r"\b(my|this|the)\s+(system|machine|pc|server|host|process|cpu|ram|disk|network)\b", lower):
            return "task"

        # Anything with a file extension mentioned → task
        if re.search(r"\.(pdf|txt|log|sh|py|html|css|js|json|xml|csv|docx|xlsx|png|jpg|zip|tar|gz)\b", lower):
            return "task"

        # Short inputs default to task — agent should try to act, not just chat
        return "task" 
...
class CLI:
    # Prompt is built dynamically to show cwd — see _get_prompt()
    PROMPT = ""  # overridden by _get_prompt()

    def _get_prompt(self):
        try:
            _sudo_user = os.environ.get("SUDO_USER")
            # Use the invoking user's home for ~ substitution, not root's
            home = os.path.expanduser(f"~{_sudo_user}") if _sudo_user                    else os.path.expanduser("~")
            display = self.cwd
            if display == home:
                display = "~"
            elif display.startswith(home + "/"):
                display = "~/" + display[len(home)+1:]
        except Exception:
            display = self.cwd
        _sudo_user = os.environ.get("SUDO_USER", "")
        _ctx = c("dim", f"[{_sudo_user}]") if (self.run_as_user and _sudo_user) else ""
        return (c("green", "hackers") + c("dim", "@") +
                c("cyan", "ai") + c("dim", ":") +
                c("yellow", display) + _ctx + c("dim", " ❯ "))

    # GUI apps / programs that must NOT run as root
    GUI_COMMANDS = {
        "firefox","chromium","chromium-browser","google-chrome","brave-browser",
        "xdg-open","nautilus","thunar","dolphin","nemo","pcmanfm",
        "mousepad","gedit","kate","pluma","xed",
        "vlc","mpv","totem","rhythmbox","audacity",
        "gimp","inkscape","libreoffice","evince","okular","zathura",
        "code","vscode","atom","sublime","subl",
    ...
    SLASH_COMMANDS = {
        ...
        "/user":    "Switch execution context: /user root | /user kali",
    ...
    def __init__(self):
        ...
        self.run_as_user = False             # /user toggle — run cmds as SUDO_USER
        # Working directory — defaults to the invoking user's home/cwd, not root
        _sudo_user = os.environ.get("SUDO_USER")
        if _sudo_user:
            self.cwd = os.path.expanduser(f"~{_sudo_user}")
        else:
            self.cwd = os.getcwd()
    ...
    def _handle_slash(self, cmd: str) -> bool:
        ...
        if slug == "/user":
            _sudo_user = os.environ.get("SUDO_USER", "kali")
            if arg.lower() in ("root", "sudo"):
                self.run_as_user = False
                print(c("red",   f"  ● Switched to ROOT"))
            elif arg and arg.lower() in (_sudo_user, "user", "normal"):
                self.run_as_user = True
                print(c("green", f"  ● Switched to USER: {_sudo_user}"))
            elif arg:
                print(c("red", f"  Unknown user: {arg}. Available: root, {_sudo_user}"))
            else:
                # No arg — show current context + available users
                cur_name = _sudo_user if self.run_as_user else "root"
                cur_col  = "green" if self.run_as_user else "red"
                print()
                print(c("yellow", "  ╔══ USER CONTEXT " + "═"*46))
                print(f"  {c('yellow','║')}  Current : {c(cur_col, f'● {cur_name}')}")
                print(c("yellow", "  ╠══ AVAILABLE USERS " + "═"*43))
                # root is always available
                root_marker = c("cyan", "◀ active") if not self.run_as_user else ""
                print(f"  {c('yellow','║')}  {c('red','root'):<30}  /user root    {root_marker}")
                # real user
                user_marker = c("cyan", "◀ active") if self.run_as_user else ""
                print(f"  {c('yellow','║')}  {c('green', _sudo_user):<30}  /user {_sudo_user}  {user_marker}")
                # scan /etc/passwd for other real users (uid 1000+)
                try:
                    with open("/etc/passwd") as _pw:
                        for _line in _pw:
                            _parts = _line.strip().split(":")
                            if (len(_parts) >= 7 and _parts[6] not in ("/usr/sbin/nologin","/bin/false","/sbin/nologin")
                                    and int(_parts[2]) >= 1000 and _parts[0] != _sudo_user and _parts[0] != "root"):
                                print(f"  {c('yellow','║')}  {c('dim', _parts[0]):<30}  /user {_parts[0]}")
                except Exception:
                    pass
                print(c("yellow", "  ╚" + "═"*62))
                print()
    ...
    def process(self, user_input: str):
        # Inject current working directory and real home into profile for planner
        ...
        # Purely informational plan — but if planner gave steps, still execute them
        if plan.get("intent") == "informational":
            if not plan.get("steps"):
                ...
                return
            # Has steps — fall through to execute (planner knows best)
        ...
        engine._cwd         = self.cwd         # propagate working directory
        engine._user_input  = enriched         # propagate original user request
    ...
    def run(self):
        ...
        while True:
            ...
            # ── Slash commands must be handled FIRST ─────────────
            if user_input.startswith("/"):
                if not self._handle_slash(user_input):
                    print(c("red", f"  Unknown command: {user_input}. Type /help."))
                continue

            # ── Direct shell execution — bypass planner for pure shell cmds ──
            ...
            _DIRECT_CMDS = {
                ...
                "systemctl","service","journalctl","dmesg","lscpu","lsusb",
                "lspci","lsmod","modinfo","tar","zip","unzip","gzip","gunzip",
                "nano","vim","vi","nvim","gedit","code","python3","python",
                "node","php","ruby","bash","sh","zsh","fish","clear","reset",
            }
            _first = _words[0].lower() if _words else ""
            # Check if it's a pure shell command (single cmd or pipeline/redirection)
            _is_direct = (
                ...
                ["install","download","create","make","build","setup","configure",
                 "how","what","why","explain","describe","tell me"]
            )
            if _is_direct and _first not in ("cd", "recon","note","notes","save","dryrun"):
                ex      = CommandExecutor()
                _cmd    = user_input.strip()
                # GUI apps or run_as_user mode → wrap with su -c
                _is_gui = _first in self.GUI_COMMANDS
                if _is_gui or self.run_as_user:
                    _cmd = CommandExecutor.as_user(_cmd)
            ...
            # ── Handle `cd` to change working directory ───────────
            if _words and _words[0].lower() == "cd":
                target = " ".join(_words[1:]).strip() if len(_words) > 1 else ""
                if not target or target == "~":
                    _sudo_user = os.environ.get("SUDO_USER")
                    target = os.path.expanduser(f"~{_sudo_user}") if _sudo_user \
                             else os.path.expanduser("~")
