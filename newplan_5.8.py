...
VERSION       = "4.9.0"   # bump this to bust __pycache__
...
13. CRITICAL — Tasks that CREATE FILES with content (HTML, scripts, config files, etc.):
    - NEVER use shell echo/printf/heredoc/tee — content in JSON strings breaks parsing
    - ALWAYS use type="python" and write with open(path,"w").write("...content...") in command
    - Pattern: step1=mkdir -p (type=command), step2=write file (type=python)
    - The python command field should contain a complete self-contained python3 script
...
depends_on: list of step IDs this step must wait for. Empty [] means it can run in parallel.
    ...
    def _expand_file_step(self, step: dict, user_input: str,
                          ...
                          cwd: str = "/root") -> dict:
        """
        For a python step that writes a file, generate the actual file content
        in a separate focused API call (no JSON overhead — just plain text).
        Then wrap it into a proper python open().write() command.
        """
        desc    = step.get("description", "")
        cmd     = step.get("command", "") or ""
        # Extract target path if already in command
        path_m  = re.search(r"""['"](/[^'"]+\.[a-z]{2,5})['"]""", cmd)
        path    = path_m.group(1) if path_m else None

        if not path:
            # Ask LLM what path to write — pass CWD so it resolves correctly
            agent   = FreeLLM(model=model)
            path    = agent.ask(
                f"What is the full absolute file path to create for this task?\n"
                f"Task description: {desc}\n"
                f"Original user request: {user_input}\n"
                f"Current working directory (CWD): {cwd}\n"
                f"IMPORTANT: Use the CWD to resolve relative paths. "
                f"If user said \'here\' or didn\'t specify a path, use the CWD.\n"
                f"Reply with ONLY the absolute path (e.g. /home/kali/Desktop/web-app/login.html). Nothing else."
            ).strip().split("\n")[0].strip().strip("'\"")

        # Generate the file content — completely separate call, plain text output
        agent   = FreeLLM(model=model)
        content = agent.ask(
            f"Generate the complete file content for: {desc}\n"
            f"Original request: {user_input}\n"
            f"File path: {path}\n"
            f"Output ONLY the raw file content — no explanation, no markdown fences, "
            f"no preamble. Just the content that should be written to the file."
        ).strip()
        ...
        # Build a self-contained python script to write the file
        ...
        python_script = (
            ...
            f'print(f"[✓] Written: {path} ({{len(content)}} bytes)")'
        )

        step["command"] = python_script
        step["type"]    = "python"
        return step
    ...
    def plan(self, user_input: str, history: list, profile: dict,
        ...
        base_parts.append(
            ...
            f"IMPORTANT: For steps that write file content (HTML/CSS/scripts/configs), "
            f"use type=python with command=PLACEHOLDER — content will be filled separately. "
            f"Do NOT inline file content in the JSON."
        )
        prompt = "\n".join(base_parts)
        ...
        for attempt in range(1, 4):
            try:
                ...
                if plan and isinstance(plan.get("steps"), list) and plan["steps"]:
                    # ── Phase 2: expand file-writing python steps ──
                    if self._needs_content_expansion(user_input):
                        expanded_steps = []
                        for step in plan["steps"]:
                            if step.get("type") == "python":
                                cmd = step.get("command", "") or ""
                                # If the command looks like a placeholder or is very short
                                if (not cmd or "PLACEHOLDER" in cmd.upper()
                                        or len(cmd) < 60
                                        or "open(" not in cmd):
                                    print(c("dim", f"  [Planner] Expanding content for: {step.get('description', 'file step')}"))
                                    step = self._expand_file_step(
                                        step, user_input, model,
                                        cwd=profile.get("cwd", "/root")
                                    )
                ...
                prompt += (
                    ...
                    "For file-writing steps use type=python with command=PLACEHOLDER. "
...
class CLI:
    ...
    def process(self, user_input: str):
        # Inject current working directory into profile for planner context
        ...
        while True:
            ...
            if _words and _words[0].lower() == "cd":
                ...
                if not target or target == "~":
                    _sudo_user = os.environ.get("SUDO_USER")
                    target = os.path.expanduser(f"~{_sudo_user}") if _sudo_user                              else os.path.expanduser("~")
