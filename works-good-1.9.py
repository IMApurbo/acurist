...
"""
    ...
    def _get_action_list_prompt(self):
        """Return the list of available actions for the agent prompt"""
        return """
...
[WEB DIRECTORY & FILE ENUM]
  ...
  gobuster vhost -u <url> -w /usr/share/wordlists/SecLists/Discovery/Web-Content/common.txt
  ffuf -u <url>/FUZZ -w /usr/share/wordlists/dirbuster/directory-list-2.3-medium.txt -mc 200,301,302,403
...
[SQL INJECTION]
  ...
  curl -s "<url>?id=1 UNION SELECT NULL,NULL,NULL--"
  # Time-based blind:
  curl -s "<url>?id=1' AND SLEEP(5)--" -w "%{{time_total}}"
  # Automated:
...
[OPEN REDIRECT]
  ...
  curl -I "<url>?next=//evil.com"
  ffuf -u "<url>?url=FUZZ" -w /usr/share/wordlists/SecLists/Fuzzing/open-redirects.txt

[CSRF]
  # Check for missing CSRF tokens:
  curl -s -X POST "<url>/action" -d "param=value" -H "Cookie: session=<token>"

[RACE CONDITION]
  # Use turbo intruder concept via curl parallel:
  seq 1 20 | xargs -P 20 -I{{}} curl -s -X POST "<url>/redeem" -d "code=PROMO123" -H "Cookie: session=<token>"

[BUSINESS LOGIC]
  # Negative price / quantity:
  curl -s -X POST "<url>/cart" -d "item=1&qty=-1&price=-99.99" -H "Cookie: session=<token>"
  # Skip payment step:
  curl -s -X POST "<url>/order/confirm" -H "Cookie: session=<token>"

[AUTH & SESSION]
  # JWT decode and tamper:
  echo "<jwt_header>.<jwt_payload>" | base64 -d
  # Brute force login:
...
[HEADERS & MISCONFIG]
  curl -I <url>
  curl -s -X OPTIONS <url> -v
  curl -s <url> --path-as-is -v

[SERVICES]
  # FTP anonymous:
  ftp <target>  (user: anonymous)
  # SMB enum:
  enum4linux -a <target>
...
0. PRE-FLIGHT CHECKS (always do this first for tool/file-dependent tasks):
   - Before using any wordlist path, verify it exists:
     <action>{{"type": "command", "command": "ls /usr/share/wordlists/dirbuster/ 2>/dev/null || echo NOT_FOUND"}}</action>
   - Before running a tool, check it's installed:
     <action>{{"type": "command", "command": "which gobuster sqlmap ffuf dalfox nmap 2>/dev/null"}}</action>
   - Use observation results to pick the correct available path/tool
   - If a wordlist isn't found in the expected path, check alternatives:
     ls /usr/share/wordlists/
     ls /usr/share/wordlists/SecLists/Discovery/Web-Content/ 2>/dev/null
     find /usr/share/wordlists -name "*.txt" 2>/dev/null | head -20
   - Only proceed to the actual task after confirming what's available

1. MISSING INFO → Ask before acting:
   - "test xss" with no URL → ask for the URL/endpoint
   - "scan" with no target → ask for IP/domain
   - "brute force" with no target or wordlist preference → ask
   - Use: <question>What is the target URL/IP?</question>
   - ONE question at a time, be specific

2. INFO AVAILABLE → Act immediately with correct commands:
   - Use real Kali tools from the knowledge base above
   - Prefer: gobuster, ffuf, sqlmap, dalfox, nmap, curl — over internal actions
   - Internal actions (port_scan, dir_bruteforce etc.) = last resort only

3. INFORMATIONAL ("what is X", "tell me about X", "explain X", "how does X work"):
   - Answer from your expert knowledge using <reply> tag. NO commands, NO actions.
   - Example: <reply>sqlmap is an open-source penetration testing tool that automates SQL injection detection...</reply>
   - Then emit <done>
...
FORMAT:
  ...
  <reply>plain text answer for informational questions</reply>
...
{action_list}"""
            ...
            # Check for plain text reply (informational answers or narration)
            reply_text = self.extract_reply(response)
            if reply_text and not actions:
                md = Markdown(reply_text)
                self.console.print(Panel(md, title="[bold cyan]AI[/bold cyan]", border_style="cyan"))
                self.task_active = False
                break
            elif reply_text and actions:
                # Narration alongside actions — just print it
                self.console.print(f"\n[bold cyan]AI:[/bold cyan] {reply_text}\n")
            ...
            if actions:
                ...
                for idx, action in enumerate(actions, 1):
                    ...
                    # Smart success: distinguish syntax/infra errors from tool-level results
                    # A command is only truly "failed" if it couldn't run at all (returncode -1)
                    # or printed usage/error indicators — not just because a URL returned 404
