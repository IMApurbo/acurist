...
Does this request involve writing/saving content to a file? Answer only YES or NO."""
                ...
                if 'YES' in second_pass:
                    ...
                    # Extract filename — first try current message, then conversation history
                    fname_match = re.search(r'[\w./\\-]+\.\w+', user_input)
                    if fname_match:
                        target_file = fname_match.group(0)
                    else:
                        # Look back through history for the last mentioned file
                        target_file = None
                        ...
                        self.console.print(f"[dim]Resolved file from history: {target_file}[/dim]")

                    # Generate ONLY the raw content — no meta-commentary
...
Output only the raw file content now:"""
                    ...
                    # Write directly without relying on action parsing
            ...
            if actions:
                ...
                BINARY_EXTS = {'.pdf', '.xlsx', '.xls', '.docx', '.doc', '.png', '.jpg', '.jpeg', '.gif', '.zip', '.tar', '.gz'}
                redirected = []
                for a in actions:
...
if __name__ == "__main__":
    main()
"""capable to crate python file for that work hich cant be done directly uisng cli tool, it can run the file , and after execution delte that , and if any pakaeges need to work , it automatically install nessassary module"""
