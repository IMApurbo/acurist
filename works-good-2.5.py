...
class AutonomousAI:
    def __init__(self):
        ...
        self.history_file = self.workspace / '.acurist_history.json'
        self.conversation_history: list = []
        # Clear stale history on fresh start
        try:
...
Generate the complete file content:"""
    ...
    def execute_python_code(self, code, description="temp task"):
        """Write a temp Python script, execute it, then delete it. Auto-installs missing modules."""
        import time
        self.console.print(f"\n[cyan]🐍 Running Python code:[/cyan] {description}\n")

        tmp_path = self.workspace / f"_acurist_tmp_{int(time.time())}.py"

        # Import name → pip package name mapping
        PKG_MAP = {
            'fpdf': 'fpdf2', 'fpdf2': 'fpdf2',
            'PIL': 'Pillow', 'PIL.Image': 'Pillow',
            'cv2': 'opencv-python',
            'docx': 'python-docx',
            'pptx': 'python-pptx',
            'openpyxl': 'openpyxl',
        ...
        def pre_install(src):
            """Install any packages needed by the script before running it"""
            mods = extract_imports(src)
            for mod in mods:
                pip_pkg = PKG_MAP.get(mod)
                if not pip_pkg:
                    continue
                ...
                if check.returncode == 0:
                    continue
                self.console.print(f"[yellow]📦 Installing {pip_pkg}...[/yellow]")
                result = subprocess.run(
                    f"pip3 install {pip_pkg} -q --break-system-packages 2>&1 || pip3 install {pip_pkg} -q 2>&1",
                    shell=True, capture_output=True, text=True
                )
                if result.returncode == 0:
                    self.console.print(f"[green]✓ Installed {pip_pkg}[/green]")
                else:
                    self.console.print(f"[red]✗ Failed to install {pip_pkg}: {result.stdout.strip()}[/red]")

        def clean_code(src):
            """Remove os.system pip install lines — handled externally now"""
            return '\n'.join(
                l for l in src.splitlines()
                if not ('os.system' in l and 'pip' in l and 'install' in l)
            )
        ...
        def _run(src):
            ...
            self.console.print(Panel(syntax, title="[cyan]Generated Script[/cyan]", border_style="cyan"))
            process = subprocess.Popen(
                f"python3 {tmp_path}",
                shell=True,
                stdout=subprocess.PIPE,
        ...
        try:
            ...
            # Pre-install all required packages before running
            pre_install(code)

            returncode, stdout = _run(code)

            # If still failing with ModuleNotFoundError, install and retry (catches unlisted packages)
            retries = 0
            while returncode != 0 and retries < 3:
                missing = re.search(r"ModuleNotFoundError: No module named '([^']+)'", stdout)
                if not missing:
                    break
                pkg = missing.group(1).split('.')[0]
                pip_name = PKG_MAP.get(pkg, pkg)
                self.console.print(f"\n[yellow]📦 Auto-installing: {pip_name}[/yellow]")
                inst = subprocess.run(
                    f"pip3 install {pip_name} -q --break-system-packages 2>&1 || pip3 install {pip_name} -q 2>&1",
                    shell=True, capture_output=True, text=True
                )
                if inst.returncode == 0:
                    self.console.print(f"[green]✓ Installed {pip_name} — retrying...[/green]")
                    returncode, stdout = _run(code)
                else:
                    self.console.print(f"[red]✗ Could not install {pip_name}[/red]")
                    break
                retries += 1
            ...
            if success:
                ...
                self.console.print(f"\n[red]✗ Script exited with code {returncode}[/red]")

            return {'stdout': stdout, 'stderr': '', 'returncode': returncode, 'success': success}
            ...
            try:
                tmp_path.unlink()
                self.console.print(f"[dim]🗑 Temp script deleted[/dim]")
            except:
                pass
        ...
        try:
            if action_type == 'command':
                ...
                return self.execute_python_code(
                    action.get('code', ''),
                    action.get('description', 'temp task')
                )
        ...
        try:
            if self.history_file.exists():
                with open(self.history_file, 'r') as f:
                    data = json.load(f)
                    self.console.print(f"[dim]📖 Loaded {len(data)} previous conversation(s)[/dim]")
...
<action>{"type": "execute_python", "description": "create a PDF about a cow", "code": "import fpdf\n..."}</action>
"""
...
4b. FILE OPERATIONS — always use actions, never reply with the content:
   - "create a file" → use create_file action with a meaningful description
   - "write X to/on/in a file" → use write_file action with the actual content
   - "write a story/poem/code on abc.txt" → write_file with path=abc.txt, content=the full story/poem/code
   - "create a file AND write content" → ONE write_file action is enough (skip separate create_file)
   - NEVER put file content inside a <reply> tag — it must go into a write_file or edit_file action
   - After writing, confirm with a short <reply> like "Done — story written to abc.txt"

4d. TASKS NEEDING PYTHON CODE (no suitable CLI tool exists):
   - "create a PDF", "generate an image", "make an Excel file", "create a ZIP", "resize image" etc.
   - Use execute_python action: write complete working Python code, run it, file is auto-deleted after
   - Common libraries available: fpdf2/reportlab (PDF), Pillow (images), openpyxl (Excel), zipfile, requests
   - First check/install library, then run the script:
     <action>{{"type": "command", "command": "pip3 install fpdf2 -q 2>/dev/null"}}</action>
     <action>{{"type": "execute_python", "description": "create PDF", "code": "from fpdf import FPDF\npdf = FPDF()\npdf.add_page()\npdf.set_font('Arial', size=12)\npdf.cell(200, 10, txt='Hello', ln=True)\npdf.output('story.pdf')"}}</action>
   - Code must be complete and self-contained — save output files to current directory (no path prefix)
   - After running, confirm what file was created

   !! CRITICAL FORMAT RULES — ALWAYS ENFORCE !!
   - .pdf  files → ALWAYS execute_python with fpdf2/reportlab. NEVER create_file or write_file
   - .pptx files → ALWAYS execute_python with python-pptx. NEVER create_file or write_file
   - .xlsx files → ALWAYS execute_python with openpyxl. NEVER create_file or write_file
   - .docx files → ALWAYS execute_python with python-docx. NEVER create_file or write_file
   - .png/.jpg/.gif → ALWAYS execute_python with Pillow. NEVER create_file or write_file
   - .zip/.tar → ALWAYS execute_python with zipfile/tarfile. NEVER create_file or write_file
   - create_file and write_file are ONLY for plain text formats: .txt .py .js .html .md .csv .json etc.
   - "presentation", "powerpoint", "ppt", "slides" → always means .pptx → always use execute_python with python-pptx
   - You are a general-purpose assistant FIRST, pentesting assistant SECOND
   - Creative writing, stories, poems, essays → write_file or <reply> with the content
   - Coding help, explanations, calculations → answer directly
   - Never refuse innocent requests by citing security/ethics concerns
   - "write a story about a farmer" is just creative writing — do it, write it to the file if one is specified
...
{action_list}"""
            ...
            if reply_text and not actions:
                # Ask AI if this should have been a file write — let it decide
                second_pass = self.bot.ask(
                    f"""The user said: "{user_input}"
Does this request involve writing/saving content to a file? Answer only YES or NO."""
                ).strip().upper()

                if 'YES' in second_pass:
                    self.console.print(f"\n[yellow]Generating file content...[/yellow]")

                    # Detect binary format keywords even without explicit filename
                    BINARY_KEYWORDS = {
                        'ppt': '.pptx', 'powerpoint': '.pptx', 'presentation': '.pptx', 'slides': '.pptx',
                        'pdf': '.pdf',
                        'excel': '.xlsx', 'spreadsheet': '.xlsx',
                        'word': '.docx',
                        'image': '.png', 'photo': '.png', 'picture': '.png',
                        'zip': '.zip',
                    }
                    PKG_FOR_EXT = {
                        '.pptx': 'python-pptx', '.pdf': 'fpdf2', '.xlsx': 'openpyxl',
                        '.docx': 'python-docx', '.png': 'Pillow', '.zip': 'zipfile'
                    }
                    ui_lower = user_input.lower()

                    # Extract filename from message or history
                    fname_match = re.search(r'[\w./\\-]+\.\w+', user_input)
                    if fname_match:
                        target_file = fname_match.group(0)
                        target_ext = Path(target_file).suffix.lower()
                    else:
                        # Check for binary format keyword
                        target_ext = next((ext for kw, ext in BINARY_KEYWORDS.items() if kw in ui_lower), None)
                        if target_ext:
                            target_file = f"output{target_ext}"
                        else:
                            # Fall back to last file in history
                            target_file = None
                            for turn in reversed(self.conversation_history):
                                hm = re.search(r'[\w./\\-]+\.\w+', turn.get('user', '') + ' ' + turn.get('summary', ''))
                                if hm:
                                    target_file = hm.group(0)
                                    break
                            target_file = target_file or 'output.txt'
                            target_ext = Path(target_file).suffix.lower()

                    BINARY_EXTS = {'.pdf', '.xlsx', '.xls', '.docx', '.doc', '.pptx', '.ppt',
                                   '.png', '.jpg', '.jpeg', '.gif', '.zip', '.tar', '.gz'}

                    if target_ext in BINARY_EXTS:
                        # Generate Python code to create the binary file
                        lib = PKG_FOR_EXT.get(target_ext, 'appropriate library')
                        history_ctx = self._format_conversation_history()
                        code = self.bot.ask(
                            f"""Write complete Python code to create a '{target_file}' file.
{history_ctx}
User request: {user_input}
Use {lib}. Do NOT use os.system to install — assume library is pre-installed.
Save output as '{target_file}' in current directory.
Output ONLY raw Python code, no markdown fences, no explanation."""
                        ).strip()
                        # Strip markdown fences if present
                        code = re.sub(r'^```\w*\n?', '', code)
                        code = re.sub(r'\n?```$', '', code)
                        result = self.execute_python_code(code, f"create {target_file}")
                        self.recent_observations.append({
                            "step": step_count,
                            "action": {"type": "execute_python", "path": target_file},
                            "result": result,
                    ...
                    # Plain text file write
                    history_ctx = self._format_conversation_history()
                    raw_content = self.bot.ask(
                        f"""You are writing content for a file. Output ONLY the raw content — no explanation, no preamble, no commentary.
{history_ctx}
User request: {user_input}
Target file: {target_file}
Output only the raw file content now:"""
                    ).strip()
                    success, err = self.write_file(target_file, raw_content)
                    self.recent_observations.append({
                        "step": step_count,
                        "action": {"type": "write_file", "path": target_file},
                        "result": {"success": success},
                        "success": success,
                        "time": datetime.now().isoformat()
                    })
                    if success:
                        self.console.print(f"\n[bold cyan]💬 Acurist:[/bold cyan] Content written to {target_file}\n")
                    self.task_active = False
                    break
            ...
            if actions:
                # Intercept create_file/write_file for binary formats — redirect to execute_python
                BINARY_EXTS = {'.pdf', '.xlsx', '.xls', '.docx', '.doc', '.pptx', '.ppt', '.png', '.jpg', '.jpeg', '.gif', '.zip', '.tar', '.gz'}
                redirected = []
                for a in actions:
                    if a.get('type') in ('create_file', 'write_file'):
                        ext = Path(a.get('path', '')).suffix.lower()
                        if ext in BINARY_EXTS:
                            self.console.print(f"[yellow]⚠ {ext} is a binary format — switching to execute_python[/yellow]")
                            # Ask AI to generate the proper Python code for this file
                            lib_map = {
                                '.pdf': 'fpdf2', '.xlsx': 'openpyxl', '.xls': 'openpyxl',
                                '.docx': 'python-docx', '.doc': 'python-docx',
                                '.pptx': 'python-pptx', '.ppt': 'python-pptx',
                                '.png': 'Pillow', '.jpg': 'Pillow', '.jpeg': 'Pillow', '.gif': 'Pillow',
                                '.zip': 'built-in zipfile', '.tar': 'built-in tarfile', '.gz': 'built-in tarfile'
                            }
                            lib = lib_map.get(ext, 'appropriate library')
                            fname = a.get('path', f'output{ext}')
                            content_hint = a.get('content', a.get('description', ''))
                            code = self.bot.ask(
                                f"""Write complete Python code to create a {ext} file named '{fname}'.
Content/description: {content_hint}
Use {lib}. Install it first with pip3 if needed (use os.system).
Save the file as '{fname}' in the current directory.
Output ONLY the Python code, no explanation, no markdown fences."""
                            ...
                            if code.startswith('```'):
                                code = re.sub(r'^```\w*\n?', '', code)
                                code = re.sub(r'\n?```$', '', code)
                            redirected.append({'type': 'execute_python', 'description': f'create {fname}', 'code': code})
                            continue
                    redirected.append(a)
                actions = redirected
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                # Record turn in conversation history (keep last 10) and persist
